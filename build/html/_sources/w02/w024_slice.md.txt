...
这个求和函数 **只能接受`[3]int`类型，其他的都不支持**。 再比如，

```go
a := [3]int{1, 2, 3}
```

数组a中已经有三个元素了，我们 **不能再继续往数组a中添加新元素**了，所以就有了slice类型。
...
<img src="asserts/slice_01.png" alt="slice_01" style="zoom: 33%;" />   
...
由于 **切片是引用类型**，所以a和b其实都指向了同一块内存地址。修改b的同时a的值也会发生变化。
